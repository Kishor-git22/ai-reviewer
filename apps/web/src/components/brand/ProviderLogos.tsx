/**
 * Real provider marks for the "powered by" marquee, sourced from Simple
 * Icons (https://simpleicons.org, CC0)  single-color glyphs distributed
 * specifically for referencing a brand/technology, as opposed to embedding
 * the companies' actual proprietary multi-color logo files. Brand names and
 * marks remain the property of their respective owners.
 *
 * AI21 has no entry in Simple Icons (checked their full ~3,300-icon dataset,
 * not just a slug guess), so it gets a plain typographic mark below instead
 * of either a missing image or someone else's logo standing in for it.
 */
...
export function NvidiaIcon({ className }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
...
export function AI21Mark({ className }: IconProps) {
  return (
    <span
      className={className}
      style={{ fontSize: 8, fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1 }}
    >
      AI21
    </span>
  )
...
export const PROVIDER_ICONS: Record<string, (props: IconProps) => React.ReactElement> = {
  Meta: MetaIcon,
  NVIDIA: NvidiaIcon,
  Google: GoogleIcon,
